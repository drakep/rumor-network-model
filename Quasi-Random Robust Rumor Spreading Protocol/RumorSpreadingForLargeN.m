...
n = 1; % number of times to run the random walk
RC = .5;%Random Chance Value, must be less than 1
PP = .1; % Pause for animation
%h = figure(1);
A = zeros(N);
Cc = zeros(N);
A = gnp(N,RC); %Create a an adj. matrix populated randomly
figure('visible','on')
while (prod(sum(A)) == 0) & (numel(graphtraverse(sparse(A),3)) ~= N)
    A = gnp(N,RC);
    imagesc(A)
    pause(.01)
end
...
B(myrandint(1,1,[1:numel(B)])) = 1; % uncomment for more than one starting node
